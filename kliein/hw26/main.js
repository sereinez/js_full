...
export function trackButtonClick(buttonId, message) {
  const button = document.getElementById(buttonId);
  
  if (!button) {
    console.warn(`Елемент з ID "${buttonId}" не знайдено.`);
    return;
  }
...
// trackButtonClick('myButton', 'Button clicked!');
...
// createTestList()
/**
 * @param {string} listSelector - CSS селектор для списку (ul або ol).
 */
export function setupEventDelegation(listSelector) {
  const listElement = document.querySelector(listSelector);

  if (!listElement) {
    console.warn(`Елемент за селектором "${listSelector}" не знайдено.`);
    return;
  }

  listElement.addEventListener('click', (event) => {
    // Перевіряємо, чи клік відбувся саме на елементі <li> всередині нашого списку
    const listItem = event.target.closest('li');

    // Перевірка, що елемент знайдено і він належить поточному списку
...
export { trackButtonClick, trackMousePosition, setupEventDelegation }
