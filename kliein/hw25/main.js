...
function createDomElement(tagName, textContent, container) {
  ...
  return element; 
console.log(`Created element: <${tagName}> with text: "${textContent}" and added to container.`);
} 
...
function setUserInfoCookie(key, value) {
    ...
    const expires = new Date(Date.now() + 10 * 1000).toUTCString(); // Встановлюємо термін дії на 10 секунд
    document.cookie = `userInfo=${encodedValue}; expires=${expires}; path=/`;
    console.log(`Cookie set: userInfo=${encodedValue}`);

  // code here
}
setUserInfoCookie('language', 'en'); // Встановлюємо cookie з інформацією про мову інтерфейсу
